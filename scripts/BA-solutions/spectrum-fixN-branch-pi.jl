...
# "ground state" in the momentum p=pi branch
ψph0 = ph_excitation(ψ0, [0], fill(-1, N))
ψph1 = ph_excitation(ψph0, [0], [-1]);
Eph0 = energy(ψph0)
scale_E(x::Float64) = (x - Eph0) / ΔE
scale_E(ψ::LLBAState) = (energy(ψ) - Eph0) / ΔE
...
for qL in ph_actions
    for qR in ph_actions
        push!(states, ph_excitation(ψph0, qL, qR))
    end
end

msk = scale_E.(energy.(states)) .< 5.0
scale_E.(energy.(states))
...
holomorphic_ovlps_gs = [kacmoody(ψi, ψph0, :holomorphic, v, K) for ψi in states]
anti_holomorphic_ovlps_gs = [kacmoody(ψi, ψph0, :antiholomorphic, v, K) for ψi in states]
holomorphic_ovlps_1 = [kacmoody(ψi, ψph1, :holomorphic, v, K) for ψi in states]
anti_holomorphic_ovlps_1l = [kacmoody(ψi, ψph1, :antiholomorphic, v, K) for ψi in states]
...
sc1_orig = scatter!(ax1, [N], [0], color=:gray50, marker=:star5, markersize=15)
sc1_main, sc1_mappedRp, sc1_mappedRm, sc1_mappedLp, sc1_mappedLm = plot_spect(ax1, ψph0, holomorphic_ovlps_gs, anti_holomorphic_ovlps_gs)
...
sc2_main, sc2_mappedRp, sc2_mappedRm, sc2_mappedLp, sc2_mappedLm  = plot_spect(ax2, ψph1, holomorphic_ovlps_1, anti_holomorphic_ovlps_1l)
sc2_orig = scatter!(ax2, [N+1], [scale_E(energy(ψph1))], color=:gray50, marker=:star5, markersize=15)
...
@show fig
Makie.save("scripts/BA-solutions/fig-ba-spect-branch-pi.pdf", fig)
